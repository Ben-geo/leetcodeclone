...
import { Firestore, arrayRemove, arrayUnion, doc, getDoc, runTransaction, updateDoc } from "firebase/firestore";
...
const ProblemDescription:React.FC<ProblemDescriptionProps> = ({problem,_solved}) => {
    ...
    const handleLike = async()=>{
        if (!user){
            toast.error("you are not logged in ",{position:"top-center",theme:"dark"});
            return;
        }
        if (updating) return;
		setUpdating(true);
        await runTransaction(firestore,async(transaction)=>{
            ...
            if( userDoc.exists()&& problemDoc.exists()){
                if(liked){
                    //remove problem id from liked problem on user Document, decrement likes on problem document
         ...
         const getCurrentProblem= async ()=>{
            ...
            if (docSnap.exists()){
                ...
                setProblemDifficultyClass(
                    problem.difficulty==="Easy"?"bg-olive text-olive":problem.difficulty==="Medium"?"bg-darl-yellow text-dark-yellow":"bg-dark-pink text-dark-pink"
